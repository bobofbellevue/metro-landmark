    ...
    const fetchStats = async () => {
        ...
        try {
            ...
            const [landlordsResult, propertiesResult, allClientsResult, usersResult, activeClientUnitsResult, leasesResult, companiesResult, unitsResult, vendorsResult] = await Promise.all([
                ...
                supabase.from('users').select('*', { count: 'exact' }).eq('is_archived', false),
            ...
            // Get user role breakdown (excluding archived users)
            const usersData = usersResult.data || [];
            const userRoleBreakdown = {
                global_admin: usersData.filter(u => u.role === 'global_admin' && !u.is_archived).length,
                company_admin: usersData.filter(u => u.role === 'company_admin' && !u.is_archived).length,
                manager: usersData.filter(u => u.role === 'manager' && !u.is_archived).length,    
                staff: usersData.filter(u => u.role === 'staff' && !u.is_archived).length,        
                tenant: tenantsCount,  // Tenants = clients with active unit assignments
                landlord: usersData.filter(u => u.role === 'landlord' && !u.is_archived).length,
