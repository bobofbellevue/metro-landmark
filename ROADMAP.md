...
| E4 Payments | Operator ledger for rent, deposits, fees, and other charge types against a lease (**Payments** in the sidebar). Create an **open charge** (due) or record money already received. Covered period is a date range aligned to the lease start day. Type and method lists are extensible per company. Proof-of-payment photos/PDFs attach to Documents. Stripe Checkout is optional when `STRIPE_SECRET_KEY` (`sk_…`) is set. |
